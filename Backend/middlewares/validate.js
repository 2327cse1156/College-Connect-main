...
export const validateSignup = [
  ...
  body("graduationYear")
    .if(body("role").equals("student"))
    .notEmpty()
    .withMessage("Graduation year is required for students")
    .isInt({ min: 2000, max: 2100 })
    .withMessage("Invalid graduation year")
    .custom((graduationYear, { req }) => {
      if (
        req.body.admissionYear &&
        parseInt(graduationYear) <= parseInt(req.body.admissionYear)
      ) {
        throw new Error("Graduation year must be after admission year");
      }
      return true;
    }),
  body("passoutYear")
    ...
    .notEmpty()
    .withMessage("Passout year is required for alumni")
...
export const validateTeamRequest = [
  ...
  body("eventType")
    .isIn(["hackathon", "project", "competition", "other"])
  ...
  body("spotsAvailable")
    .isInt({ min: 1, max: 10 })
    .withMessage("Spots must be between 1-10"),
